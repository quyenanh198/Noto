import { app } from '../app';
import { dirname, joinPath } from '../core/vault/path';
...
export async function openLink(target: string, fromPath: string | null, options: { heading?: string; newTab?: boolean } = {}): Promise<void> {
  ...
  if (!path) {
    const created = await app.vault.createUnique(target.includes('/') ? target : joinPath(fromPath ? dirname(fromPath) : '', target));
    path = created.path;
  }
...
export function registerCoreCommands(): () => void {
  const ws = () => useWorkspace.getState();
  const unregisters = [
    app.commands.register({ id: 'file:new', name: 'Create new note', hotkey: 'Mod+N', callback: () => void createNewNote() }),
    ...
    app.commands.register({ id: 'view:toggle-mode', name: 'Toggle reading view', hotkey: 'Mod+E', callback: () => ws().toggleViewMode() }),
    ...
    app.commands.register({ id: 'search:open', name: 'Search in all files', hotkey: 'Mod+Shift+F', callback: () => ws().setLeftTab('search') }),
    ...
    app.commands.register({ id: 'nav:back', name: 'Navigate back', hotkey: 'Alt+ArrowLeft', callback: () => ws().goBack() }),
    app.commands.register({ id: 'nav:forward', name: 'Navigate forward', hotkey: 'Alt+ArrowRight', callback: () => ws().goForward() }),
    app.commands.register({
      id: 'daily-note:open',
